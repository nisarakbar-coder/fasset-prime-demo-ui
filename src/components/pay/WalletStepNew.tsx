...
export function WalletStep({ paymentLink, walletStatus, onStatusChange }: WalletStepProps) {
  ...
  return (
    <div className="space-y-6">
      <div className="text-center space-y-4">
        <Wallet className="h-8 w-8 text-blue-600 mx-auto" />
        <div>
          <h3 className="text-lg font-semibold">Wallet Whitelisting Required</h3>
          <p className="text-muted-foreground">
          ...
          {verificationStep === 'video' && (
            <div className="space-y-4">
              ...
              <div className="space-y-4">
                ...
                <Button onClick={handleVideoVerification} disabled={isLoading} className="w-full">
                  {isLoading ? 'Processing Video...' : 'Submit Video for Verification'}
                </Button>
              </div>
            </div>
          )}
